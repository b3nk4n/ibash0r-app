...
namespace Bash.Tests
{
    ...
    public class BashDataTest
    {
        ...
        {
            string text = "<CHRiS> lala? da?[newline]<Lala> jaa :*[newline]<CHRiS> 3 Worte.[newline]<CHRiS> 12 Buchstaben.[newline]<CHRiS> Leicht zu sagen.[newline]<CHRiS> Schwer zu beweisen.[newline]<Lala> Jaaaa ???? :-*** [newline]<CHRiS> Ich bin Batman!!!!!11elf[newline]<Lala> bloeder penner![newline]Lala has quit IRC (******)";
            PerformDataTest(text, 10, 2);
        }

        public void BashDataSimpleWithServerJoinInfoAtBeginning()
        {
            string text = "*** blazemore (empty3@dialup-209.245.205.138.Houston1.Level3.net) has joined #ramen[newline]<LkAway1> blazemore: was ist bei Dragonball Z heute passiert?[newline]<HomerJ> die haben mit sonem typen gekämpft";
